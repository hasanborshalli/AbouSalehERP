...
class ManagedPropertyRentalController extends Controller
{
    public function store(Request $request, ManagedProperty $property, CashAccountingService $cash)
    {
        if ($property->type !== 'rental') {
            return back()->with('error', 'This property is not a rental type.');
        }

        // Cannot have two active rentals at once
        if ($property->rentals()->where('status', 'active')->exists()) {
            return back()->with('error', 'There is already an active rental for this property. End it first.');
        }
        ...
        DB::transaction(function () use ($data, $property, $monthlyRent, $ownerShare, $commission) {
        ...
        return redirect()->route('managed.show', $property)
            ->with('success', 'Rental contract created and payment schedule generated.');
    }
    ...
    public function end(Request $request, ManagedProperty $property, ManagedPropertyRental $rental)
    {
        ...
        // Set property back to active if no other active rentals
        if (!$property->rentals()->where('status', 'active')->exists()) {
            $property->update(['status' => 'active']);
        }

        return redirect()->route('managed.show', $property)
            ->with('success', 'Rental ended. Future payments removed.');
    }
