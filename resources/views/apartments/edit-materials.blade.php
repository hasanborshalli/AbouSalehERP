...
<head>
    ...
    <title>Edit Materials — {{ $project->name }}</title>
...
<body class="app-shell">
    ...
    <div class="app-shell__main">
        ...
        <main class="dashboard-content">
            ...
            <div class="alert alert--error" data-alert>
                <span class="alert__icon">✕</span>
                <span class="alert__text">{{ $errors->first() }}</span>
                <button class="alert__close" onclick="this.parentElement.remove()">✕</button>
            </div>
            @endif

            <section class="project-page" aria-label="Edit materials">

                <div class="project-page__header">
                    <h2 class="project-page__title">
                        🧱 Materials &amp; Costs
                        <span class="project-page__muted">— {{ $project->name }}</span>
                    </h2>
                    <div style="display:flex;gap:8px;">
                        <a class="project-page__back" href="{{ route('apartments.project', $project->id) }}">
                            ← Back to project
                        </a>
                    </div>
                </div>
                ...
                <div class="project-page__grid">
                    ...
                    <section class="dashboard-card" aria-label="Project materials">
                        <h3 style="margin:0 0 14px;">🧱 Materials (Inventory)</h3>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form method="post"
                                                ...
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                                    style="background:none;border:none;color:#dc2626;cursor:pointer;font-size:13px;">✕ Remove</button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
                        ...
                        @else
                        <p class="project-page__muted" style="margin-bottom:16px;">No materials added yet.</p>
                        @endif
                        ...
                        <div style="padding-top:14px;border-top:1px solid rgba(0,0,0,0.07);">
                            ...
                            <form method="post" action="{{ route('projects.materials.store', $project) }}">
                                ...
                                <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
                                    <div>
                                        <label style="font-size:12px;display:block;margin-bottom:4px;">Inventory Item</label>
                                        <select name="inventory_item_id" required
                                            style="padding:7px 10px;border-radius:7px;border:1px solid #d1d5db;font-size:13px;min-width:200px;">
                                            <option value="">Select item</option>
                                            @foreach($inventoryItems as $it)
                                            <option value="{{ $it->id }}">{{ $it->name }} (Stock: {{ $it->quantity }} {{ $it->unit }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <label style="font-size:12px;display:block;margin-bottom:4px;">Quantity Needed</label>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                            <span style="font-size:12px;color:#059669;font-weight:600;">Settled {{ $c->actual_entered_at?->format('Y-m-d') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if(!$settled)
                        ...
                        <div style="padding-top:14px;border-top:1px solid rgba(0,0,0,0.07);">
                            ...
                            <form method="post" action="{{ route('projects.costs.store', $project) }}">
                                ...
                                <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
                                    ...
                                    <div>
                                        <label style="font-size:12px;display:block;margin-bottom:4px;">Category (optional)</label>
                                    ...
                                    <div>
                                        <label style="font-size:12px;display:block;margin-bottom:4px;">Expected Amount ($)</label>
                                        <input type="number" name="expected_amount" step="0.01" min="0"
                                            placeholder="0.00" required
                                            style="padding:7px 10px;border-radius:7px;border:1px solid #d1d5db;font-size:13px;width:130px;">
                                    </div>
                                    <div>
                                        <label style="font-size:12px;display:block;margin-bottom:4px;">Notes (optional)</label>
                                        <input type="text" name="notes" placeholder="Any details..."
                                            style="padding:7px 10px;border-radius:7px;border:1px solid #d1d5db;font-size:13px;width:160px;">
                                    </div>
                                    <button type="submit"
                                        style="padding:7px 18px;border-radius:7px;background:#2563eb;color:#fff;border:none;font-size:13px;font-weight:600;cursor:pointer;">
                                        Add Cost
